...
| `TransactionalClusterLock` | `false` | By default the leadership lock is a session-scoped advisory lock multiplexed onto one shared connection per node. Set `true` to hold each lock in a transaction on its own connection instead, which is required when the lock's connections go through a pooler in transaction mode (e.g. PgBouncer `pool_mode = transaction`). |
