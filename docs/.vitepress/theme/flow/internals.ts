...
export const nodes: IntNode[] = [
  {
    ...
    detail: 'Your ordinary schema, with nothing to install in it. Only entities you declare in a mapping are captured, and every writer is seen: the application, migrations, bulk updates, admin scripts, other services sharing the database.',
  ...
  },
  {
    ...
    subs: [],
    detail: 'The write-ahead log is the source of truth: every committed row change is already in it, in commit order. Reading from the WAL is what makes dual writes and outbox tables unnecessary, and why no change is missed.',
    links: [],
  },
  {
  ...
  },
  {
    ...
    detail: 'Only one node streams: the leader, elected with a Postgres advisory lock. It opens the replication connection, answers keepalives, and reports its flushed position, which lets the server release retained WAL.',
  ...
  },
  {
    ...
    detail: 'pgoutput messages are assembled into whole transactions that only surface on commit, so rolled-back work is never seen. A streamed transaction too large to hold in memory spills to an unlogged table in the wallaby schema (or to disk, or a custom backend) and streams back out at commit.',
  ...
  },
  {
    ...
    detail: 'The storage provider maps each raw row change back to the type you already have (an EF Core entity, a Marten document, or a plain POCO) before transforms run.',
  ...
  },
  {
    ...
    detail: 'Your transform shapes each change into the output document, with the provider\'s DbContext, query session, or data source available for enrichment lookups. Results are sliced into batches of at most MaxBatchSize before delivery.',
    links: [
  ...
  },
  {
    ...
    detail: 'Independent sinks are written concurrently. A failing batch retries with backoff. If retries are exhausted, the pipeline halts rather than skipping the batch, so later changes are never delivered past a missing one.',
  ...
  },
  {
    ...
    title: 'http (webhook)',
    subs: [],
    detail: 'The HTTP sink posts signed batches to your endpoint under the same contract: the receiver upserts by id and tolerates redelivery. Any system you can reach over HTTP can be kept in sync.',
...
export const scenarios: IntScenario[] = [
  {
    ...
    steps: [
      { nodes: ['tables'], caption: 'a transaction commits, from any writer: your app, a migration, a bulk update' },
      { nodes: ['wal'], edges: ['tables-wal'], fx: 'tick', caption: 'every row change is already in the write-ahead log, in commit order' },
      { nodes: ['pub'], edges: ['wal-pub'], caption: 'the publication filters to captured tables, and to only the columns your mappings consume' },
      ...
      { nodes: ['dispatch'], edges: ['transform-dispatch'], caption: 'the dispatcher writes independent sinks concurrently, retrying failures with backoff and never skipping' },
      { nodes: SINKS, edges: ALL_SINK_DROPS, fx: 'deliver', caption: 'each sink upserts by document id, so a redelivery is harmless' },
  ...
  },
  {
    ...
    steps: [
      { nodes: ['backfill'], blue: true, caption: 'a mapping\'s backfill version changes, or a manual request arrives, and a snapshot is scheduled' },
      { nodes: ['backfill', 'tables'], edges: ['backfill-tables'], blue: true, caption: 'the keyset pager reads chunks in primary-key order, persisting progress per table so it can resume' },
      ...
      { nodes: ['route'], edges: ['materialize-route'], blue: true, caption: 'each chunk is bracketed by watermarks, so a row changed live inside the window wins over its stale snapshot copy' },
      { nodes: ['transform'], edges: ['route-transform'], blue: true, caption: 'the same transforms run, so there is no separate reindex code path to maintain' },
  ...
  },
  {
    ...
    steps: [
      ...
      { nodes: ['ack', 'slot'], edges: [...ALL_SINK_ACKS, 'ack-slot'], fx: 'flush', caption: 'the triggering transaction is acknowledged immediately, even with a huge tail still outstanding' },
      ...
      { nodes: ['materialize'], edges: ['backfill-materialize'], fx: 'deliver', blue: true, caption: 'the tail flows through the same pipeline and idempotent upserts absorb any overlap, so sinks converge eventually' },
    ],
  },
  {
    id: 'slotloss',
    ...
    steps: [
      ...
      { nodes: ['stream'], warn: ['slot'], caption: 'a fresh slot only streams forward from its creation point, so on its own the gap would be silent' },
      ...
      { nodes: SINKS, edges: ALL_SINK_DROPS, fx: 'deliver', blue: true, caption: 'idempotent upserts converge every sink and the gap is closed' },
    ],
  },
  {
    id: 'outage',
    ...
    steps: [
      ...
      { nodes: ['stream', 'assemble', 'materialize', 'route', 'transform', 'dispatch'], edges: ['slot-stream', 'stream-assemble', 'assemble-materialize', 'materialize-route', 'route-transform', 'transform-dispatch'], caption: 'decode ▸ materialize ▸ transform ▸ batch, as usual' },
      ...
      { nodes: ['slot'], warn: ['http', 'dispatch'], caption: 'if retries are exhausted, the pipeline halts rather than skipping, and the unacknowledged slot keeps retaining wal so nothing is lost' },
      ...
      { nodes: SINKS, edges: ALL_SINK_DROPS, fx: 'deliver', caption: 'the transaction is redelivered to every sink; the ones that already saw it harmlessly upsert the same documents again' },
      { nodes: ['ack'], edges: ALL_SINK_ACKS, caption: 'now every sink has accepted' },
      { nodes: ['slot', 'checkpoint'], edges: ['ack-slot', 'ack-checkpoint'], fx: 'flush', caption: 'acknowledge ▸ advance the slot ▸ save the checkpoint: the change was delayed, never dropped' },
    ],
  },
];
