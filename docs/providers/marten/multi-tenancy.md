...
scope-key selector — `ScopedByTenant()` reads the tenant straight from the change.
...
- **`ScopedByTenant()`** scopes the mapping by the change's tenant — the engine sub-groups each
  transform batch per tenant.
