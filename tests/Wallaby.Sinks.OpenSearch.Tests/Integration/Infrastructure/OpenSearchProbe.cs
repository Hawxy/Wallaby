...
/// <summary>
/// Small helper for asserting on OpenSearch documents in tests. Deliberately client-free (raw REST via
/// <see cref="HttpClient"/>) so it double-checks the sink's writes independently of the client library.
/// </summary>
public sealed class OpenSearchProbe(string endpoint) : IDisposable
{
    private readonly HttpClient _http = new() { BaseAddress = new Uri(endpoint) };

    public OpenSearchProbe(OpenSearchFixture fixture) : this(fixture.Endpoint) { }
