...
public sealed class WallabyControlClient : IAsyncDisposable
{
    ...
    // The database's wallaby.schema_version ledger value, cached once it reaches the version this
    // build knows; below that it is re-read on demand (a host may migrate the schema at any time).
        ...
        WallabySuspendOptions? options = null, CancellationToken ct = default)
    {
        ...
        return await AwaitCompletionAsync(
            options.HostGracePeriod, options.Timeout, options.Progress,
            isCompleteAsync: (state, _) => Task.FromResult(
                (state.State == WallabySuspensionState.Suspended && state.Slots.All(s => !s.ExistsOnServer))
            ...
            fallbackAsync: async token =>
            {
                ...
                await ControlOperations.FinalizeSuspensionAsync(_dataSource, PollInterval, _logger, token);
            },
            timeoutMessage: state =>
    ...
    {
        var transitioned = await ControlOperations.ResumeAsync(
    ...
    {
        var version = await GetSchemaVersionAsync(ct);
        var row = await ControlOperations.ReadAsync(
            _dataSource, includeWidening: version >= ControlContract.WideningSchemaVersion, ct);
    ...
    {
        if (_schemaVersion < ControlContract.SchemaVersion)
        {
            _schemaVersion = await ControlOperations.GetSchemaVersionAsync(_dataSource, ct);
        }
    ...
    {
        var version = await GetSchemaVersionAsync(ct);
        if (version >= ControlContract.SchemaVersion)
        {
            return;
        }
        ...
        throw new InvalidOperationException(version == 0
            ...
            : $"This database's wallaby schema is at version {version}; this operation requires version " +
              $"{ControlContract.SchemaVersion} or newer. Deploy a newer Wallaby host first — it migrates " +
        ...
        WallabyWidenOptions? options = null, CancellationToken ct = default)
    {
        ...
        await RequireCurrentSchemaAsync(ct);
        var transitioned = await ControlOperations.RequestWidenAsync(
        ...
        return await AwaitCompletionAsync(
            options.HostGracePeriod, options.Timeout, options.Progress,
            isCompleteAsync: async (state, token) =>
            {
            ...
            fallbackAsync: async token =>
            {
                ...
                await ControlOperations.WidenPublicationsDirectAsync(_dataSource, _logger, token);
            },
            timeoutMessage: _ =>
    ...
    /// <summary>
    /// The wait loop shared by every operation that persists a request and waits for the installation
    /// to converge: polls the state once per second, reports progress, completes when
    /// <paramref name="isCompleteAsync"/> says so, hands the work to this client via
    /// <paramref name="fallbackAsync"/> on every pass once the host grace period elapses, and converts
    /// deadline expiry into a <see cref="WallabyControlTimeoutException"/> carrying the last state.
    /// </summary>
    private async Task<WallabyControlState> AwaitCompletionAsync(
        TimeSpan hostGracePeriod,
        TimeSpan timeout,
        IProgress<WallabyControlState>? progress,
        Func<WallabyControlState, CancellationToken, Task<bool>> isCompleteAsync,
        Func<CancellationToken, Task> fallbackAsync,
        Func<WallabyControlState, string> timeoutMessage,
        CancellationToken ct)
    {
        ...
        try
        {
            while (true)
            {
                ...
                if (await isCompleteAsync(state, deadline.Token))
                {
                    return state;
                }

                if (Stopwatch.GetElapsedTime(start) >= hostGracePeriod)
                {
                    await fallbackAsync(deadline.Token);
                    continue;
                }
