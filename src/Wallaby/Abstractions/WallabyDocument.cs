...
/// (<c>new WallabyDocument { ["name"] = product.Name }</c>) and is consumed by sinks as an
/// <see cref="IReadOnlyDictionary{TKey,TValue}"/> with no extra allocation.
/// </summary>
public sealed class WallabyDocument : Dictionary<string, object?>
{
    /// <summary>Create an empty document.</summary>
    public WallabyDocument()
    {
    }

    /// <summary>Create an empty document with the given initial capacity.</summary>
    public WallabyDocument(int capacity) : base(capacity)
    {
    }

    /// <summary>Set a field and return the document, for fluent construction.</summary>
    public WallabyDocument Set(string key, object? value)
    {
        this[key] = value;
        return this;
    }
